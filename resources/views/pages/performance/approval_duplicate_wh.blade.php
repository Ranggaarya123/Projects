...
@section('title', 'Telkom Akses - Approval Duplicate WH')
@section('content')
    <div class="card mt-4">
        <div class="card-body text-center bg-primary text-white">
            <h3 class="my-0 fw-bold text-accent-border">Performance Approval - Duplicate WH</h3>
        </div>
    </div>
    <div class="card mt-4">
        <!-- Search Input -->
        ...
        <div class="card-body">
            ...
            <table class="table table-striped table-hover" id="table-content">
                ...
                <tbody>
                    @foreach ($duplicateWH as $item)
                        <tr data-user-id="{{ $item->user_id }}">
                            <td>{{ $item->user_id }}</td>
                            <td>{{ $item->sto }}</td>
                            <td>{{ $item->kode_wh }}</td>
                            <td>
                                @if ($item->status_duplicate === 1)
                                    <span class="text-success">Disetujui</span>
                                @elseif ($item->status_duplicate === 2)
                                    <span class="text-danger">Ditolak</span>
                                @else
                                    <form action="{{ url('/performance/duplicate-wh/' . $item->id) }}" method="POST">
                                        @csrf
