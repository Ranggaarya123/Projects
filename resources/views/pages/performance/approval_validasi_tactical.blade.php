...
@section('content')
    <div class="card mt-4">
        <div class="card-body text-center bg-primary text-white">
            <h3 class="my-0 fw-bold text-accent-border">Performance Approval - Validasi Tactical</h3>
        </div>
    </div>
    <div class="card mt-4">
        <!-- Search Input -->
        ...
        <div class="card-body">
            ...
            <table class="table table-striped table-hover" id="table-content">
                ...
                <tbody>
                    @foreach ($validasiTactical as $item)
                        <tr data-user-id="{{ $item->user_id }}">
                            <td>{{ $item->nama }}</td>
                            <td>{{ $item->user_id }}</td>
                            <td><a href="javascript:void(0);" onclick="openPdfModal('{{ asset('storage/' . $item->capture_tactical) }}', 'Capture Tactical')" class="btn"><i class="fas fa-eye"></i></a></td>
                            <td>
                                ...
                                @else
                                    <form action="{{ url('/performance/validasi-tactical/' . $item->id) }}" method="POST">
                                        @csrf
