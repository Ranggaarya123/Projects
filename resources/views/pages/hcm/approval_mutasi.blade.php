...
@section('title', 'Telkom Akses - Approval Mutasi')
@section('content')
    <div class="card mt-4">
        <div class="card-body text-center bg-primary text-white">
            <h3 class="my-0 fw-bold text-accent-border">HCM Approval - Mutasi</h3>
        </div>
    </div>
    <div class="card mt-4">
        <!-- Search Input -->
            ...
            <table  class="table table-striped table-hover" id="table-content">
                <thead class="thead-light">
                    <tr>
                        <th>NIK</th>
                        <th>Mutasi Type</th>
                        <th>Surat Keterangan Aktif</th>
                        <th>Scan KTP</th>
                        <th>Sertifikat Brevet</th>
                        <th>Surat Pakelaring</th>
                        <th>Status</th>
                        <th>Komentar</th>
                        <th>Time Created</th>
                    </tr>
                ...
                <tbody>
                    @foreach ($mutasis as $item)
                        <tr data-user-id="{{ $item->user_id }}">
                            <td>{{ $item->user_id }}</td>
                            <td>{{ $item->mutasi_type }}</td>
                            <td>
                                @if ($item->surat_keterangan_aktif)
                                    <a href="javascript:void(0);" onclick="openPdfModal('{{ asset('storage/' . $item->surat_keterangan_aktif) }}', 'Surat Keterangan Aktif')" class="btn"><i class="fas fa-eye"></i></a>
                                @else
                                    -
                                @endif
                            </td>
                            <td>
                                @if ($item->scan_ktp)
                                    <a href="javascript:void(0);" onclick="openPdfModal('{{ asset('storage/' . $item->scan_ktp) }}', 'Scan KTP')" class="btn"><i class="fas fa-eye"></i></a>
                                @else
                                    -
                                @endif
                            </td>
                            <td>
                                @if ($item->brevet)
                                    <a href="javascript:void(0);" onclick="openPdfModal('{{ asset('storage/' . $item->brevet) }}', 'Sertifikat Brevet')" class="btn"><i class="fas fa-eye"></i></a>
                                @else
                                    -
                                @endif
                            </td>
                            <td>
                                @if ($item->surat_pakelaring)
                                    <a href="javascript:void(0);" onclick="openPdfModal('{{ asset('storage/' . $item->surat_pakelaring) }}', 'Surat Pakelaring')" class="btn"><i class="fas fa-eye"></i></a>
                                @else
                                    -
                                @endif
                            </td>
                            <td>
                                @if ($item->status_mutasi === 1)
                                    <span class="text-success">Disetujui</span>
                                @elseif ($item->status_mutasi === 2)
                                    <span class="text-danger">Ditolak</span>
                                @else
                                    <form action="{{ url('/hcm/approval/mutasi/' . $item->id) }}" method="POST">
                                        @csrf
