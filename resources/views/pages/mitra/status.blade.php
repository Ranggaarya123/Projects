...
@section('content')
    <div class="card mt-4">
        <div class="card-body text-center bg-primary text-white">
            <h3 class="my-0 fw-bold text-accent-border">Semua Status Pengajuan</h3>
        </div>
    </div>
    <div class="card mt-4">
        <div class="card-body">
            ...
            <table class="table table-striped table-hover table-bordered" id="table-content">
                ...
                <tbody>
                    @foreach ($statuses as $status)
                        <tr data-user-id="{{ $status['nik'] }}" class="{{ $status['status'] === 'Done' ? 'table-success' : ($status['status'] === 'Reject' ? 'table-danger' : ($status['status'] === 'Done NDE' ? 'table-primary' : ($status['status'] === 'Approved' ? 'table-primary' : 'table-warning'))) }}">
                            <td>{{ $status['nik'] }}</td>
                            <td>{{ $status['type'] }}</td>
                            <td>
